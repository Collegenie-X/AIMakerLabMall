...
import { getPaginatedLessonInquiries } from '@/services/lessonService';
...
export default function LessonsPageContent() {
  ...
  useEffect(() => {
    ...
    fetchLessonInquiries(currentPage, itemsPerPage);
   ...
   */
  const fetchLessonInquiries = async (page: number, pageSize: number) => {
    try {
      setIsLessonLoading(true);
      const response = await getPaginatedLessonInquiries(page, pageSize);
    ...
    <Container maxWidth="lg" sx={{ py: 4 }}>
      <Paper elevation={1} sx={{ p: 3, mb: 4 }}>
        ...
        <Typography variant="body1" color="text.secondary" paragraph>
          코딩 수업 출강 및 학생 교육 프로그램에 관한 모든 문의사항을 남겨주세요. 담당자가 최대한 빠르게 답변해드립니다.
        </Typography>
      </Paper>
